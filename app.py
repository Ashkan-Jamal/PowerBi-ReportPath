...
def save_to_gdrive(file_url, file_name, gpsgate_token):
    ...
    try:
        # Download the file using the provided token
...
def get_report():
    ...
    # Get GPSGate token from Authorization header
    gpsgate_token = request.headers.get("Authorization")
    
    if not gpsgate_token:
        return jsonify({
            "error": "Authorization header required",
            "details": "Please provide your GPSGate token in the Authorization header"
        }), 401
    ...
    try:
        ...
        if response.status_code == 401:
            return jsonify({
                "error": "Invalid GPSGate token",
                "details": "The provided Authorization token is invalid or expired",
                "status_code": 401
            }), 401
        ...
        if response.status_code == 403:
            return jsonify({
                ...
                "details": "The token does not have access to this application",
